...
c
c          is time step > max steps allowed ?
...
c         
      ...
      if( output_command_file(1:10) .eq. " " ) return
      if( .not. allocated( output_step_bitmap_list ) ) then
         write(out,9200) output_command_file
         call die_gracefully
      end if
c
      word = (now_step - 1 )/30 + 1
      bit = now_step - ( word - 1 ) * 30 - 1
      output_this_step = btest( output_step_bitmap_list(word), bit ) 
