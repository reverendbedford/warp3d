...
c
c
      subroutine diexp4( ring, node_map, 
     &                   nonode, noelem, incmap, eprops, incid,
     &                   out, q_map_len, bits )
      use j_data
...
c
c
      subroutine diexp13( ring,
     &                    nonode, noelem, incmap, eprops, incid,
     &                    out, bits, newmap, nodmap,
     &                    q_map_len )
      use j_data
      use main_data, only : inverse_incidences
      implicit integer (a-z)
...
c
      dimension
     & incmap(*), eprops(mxelpr,*),
     & incid(*), bits(*), nodmap(q_map_len,*), newmap(q_map_len,*)  
c
c                     local declarations
c
      integer local_list(10), corner(3), enodes(30), 
     &        edge_table(0:3,20)
      logical dibmck, flags1(3), flags2(3), fl1, fl2
      real rzero, rone
      data rzero, rone / 0.0, 1.0 /
c
      if ( debug_driver ) write(out,9000) ring
...
c        
      do 300 elenum = 1, noelem
        if ( .not. dibmck( elenum, q_element_maps, bits ) ) go to 300
c
...
c               
     ...
     &                         fl1, fl2 
           if ( fl1 .and. fl2 ) go to 200
           if ( (.not. fl1) .and. (.not. fl2) ) go to 200
           if ( fl1 ) then
               if ( flags1(1) ) mapno = 1
               if ( flags1(2) ) mapno = 2
               if ( flags1(3) ) mapno = 3
...
c
 900  continue
      do 400 snode = 1, nonode
       q_values(snode) = rzero
       if ( .not. dibmck( snode, newmap(1,domain_type), bits ) ) 
     &   go to 400
       q_values(snode) = rone
       do i = 1, inverse_incidences(snode)%element_count
         elem = inverse_incidences(snode)%element_list(i)
...
c
c
      logical function dibmck( entry, mapvec, bits )
      implicit integer (a-z)
      dimension mapvec(*), bits(*)
c
...
c
      dimension eprops(*), edge_table(0:3,*)
c
c             local declarations
c
      dimension edge_8_table(0:2,12),edge_20_table(0:3,12),
