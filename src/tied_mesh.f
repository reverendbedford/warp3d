...
c           error checking
c 
      if(max_mpc.le.0)then
        mflag = 2
        message = 'error: max_mpc = 0 (mpc_init).'
...
c     *                                                              *
c     *  12/22/03 added epsilon_mpc to the call statement            *
c     *                                                              *
c     ****************************************************************
c
...
c             fea input file
c           mxdof = dimension of the cstmap() array, available in the
c             param_def file
...
c           mxdof = dimension of the cstmap() array, available from the
c             param_def file
c           cstmap() = given constraint flag for each dof, 
c             0=unconstrained and active dof
c           epsilon_mpc = small value to avoid saving very small
...
c
      ...
      else if(mxdof.le.0)then
        mflag = 2
        message = 'error: mxdof = 0 (tied_nodecompute).'
      else if(maxnodeperelem.le.0)then
        mflag = 2
...
c
      ...
      if(logflag.ge.debug_level)then
        ...
        write(logunit,*)'                mxdof =',mxdof
        close(unit=logunit)
      end if

      if(total_dof.gt.mxdof)then
        mflag = 2
        message = 'error: total_dof > mxdof; too many dof in the'//
     &            ' model; check the mxdof array dimension'//
     &            ' (tied_nodecompute).'
        goto 900
      end if
...
c
          ...
          if(num_tied_con_mpc.gt.max_mpc)then
            mflag = 2
