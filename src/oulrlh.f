...
c     *     put for output of the linear residual in a linear pcg    * 
c     *     algorithm.                                               * 
c     *                                                              *
c     ****************************************************************
c
...
c
         if(fmtyp.eq.1.or.fmtyp.eq.3) then
            write(out,900) formfeed,stname,pgnum,step,stpitr,iter,
     &                     hedtyp
         else if(fmtyp.eq.2.or.fmtyp.eq.4) then
            write(out,905) formfeed,stname,pgnum,step,stpitr,iter,
     &                     hedtyp
         end if          
c
c
...
c            
 900  format(a1,///8x,'structure ',a8,51x,'page no. ',i4/8x,
     &       'step no. ',i6,4x,'step iter. no. ',i6,4x,'iter. no. ',
     &       i6//9x,'nodal ',a20)
c
