...
c
      ...
      mkl_asymmetric = solver_flag .eq. 8
      mkl_symmetric  = solver_flag .eq. 7
c      
...
c
        write(out,9301) solver_flag; call die_gracefully
c
      end if ! done with calling linear solver
c
      if( local_debug ) write(out,*) ' @ 9'
      if( .not. parallel_assembly_used )  
...
c     
      ...
      end subroutine direct_routine_sparse
